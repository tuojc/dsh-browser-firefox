...
- **权限级别**：输入区盾牌按钮单选——锁定（不碰页面）/ 只读（默认，可读不可动）/ 读写（全自动）；被拒绝的调用会告诉模型当前级别与切换方法。
- **动作后自动附变化**：点击/输入/滚动等动作的结果自带 DOM 增量；导航后自动附新页快照，模型不必再花一轮 `browser_snapshot`。
- **iframe 聚合快照**：页面含 iframe 时自动合并（主帧 80% 预算），iframe 内元素用 `frame` 参数操作。
- **附件**：输入区回形针按钮添加图片（走宿主多模态通道）或文本文件（fenced 拼入消息），也可直接粘贴。
- **同组静默跟随**：助手自己打开的标签页被激活时自动接管；手动切到外部页面不打断当前工作页。
- **页面上下文会话连续**：同一窗口同一站点重开侧边栏，自动回到对应会话。
- **AMO 更新检查**：上架版本高于本地时在侧边栏提示。
