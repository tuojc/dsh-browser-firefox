...
页面被转成带编号交互元素清单的结构化文本，模型按编号操作元素；遇到文本无法表达的内容（图片、图表、验证码）时，`browser_screenshot` 把截图作为**原生图像块**直接送入对话上下文，模型直接看图。
...
会话栏的模型芯片显示当前模型，点开为按提供商分组的目录（含推理强度选择）；选择按会话生效，新会话在发送首条消息后自动应用。

### 问答卡片

助手经 `ask_user_question` 提问时，问题卡片直接出现在侧边栏底部——在浏览器里选择/填写/放弃即可，不必切到 dsh web；桥断开时自动回落给 dsh web 回答。

### 权限级别

输入区盾牌按钮选择权限级别——锁定（不碰页面）/ 只读（默认，可读不可动）/ 读写（全自动）；按钮按当前级别着色（锁定=红、读写=黄），被拒绝的调用会告诉模型当前级别与切换方法。

### 附件

输入区回形针按钮添加图片（走宿主多模态通道，张数/尺寸等限制跟随宿主投影）或文本文件（fenced 拼入消息），也可直接粘贴；消息历史中的图片经 `session/attachment` 回读渲染，点击可放大。
...
- **仅限本机回环使用**：桥地址是 `ws://` 明文，token 与页面内容在传输中不加密。如需远程部署（`dsh web --host 0.0.0.0`），请在前面自建 wss/TLS 反向代理，不要在不可信网络直接暴露 3080 端口。
