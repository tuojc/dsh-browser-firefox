...
export function accessibleName(el: Element): string {
  ...
  // label 自身为空时，选项文本常在 label 后的兄弟元素（如 icourse163：空 label 包隐藏 input + 兄弟 span 文本）
  if (el instanceof HTMLLabelElement) {
    const sibText = el.nextElementSibling?.textContent?.trim()
    if (sibText !== undefined && sibText !== '') return truncate(clean(sibText), MAX_ITEM_NAME_CHARS).text
  }
...
export function collectInteractive(root: Document | Element): Element[] {
  ...
  const pushVisible = (el: Element): void => {
    if (seen.has(el)) return
    seen.add(el)
    if (isVisible(el)) result.push(el)
  }
  // 1. 语义交互元素（a/button/input/role/onclick/tabindex/label/li 等）
  for (const el of root.querySelectorAll(INTERACTIVE_SELECTOR)) pushVisible(el)
  // 2. cursor:pointer 启发式：捕获「事件委托」的可点击元素（div/span/li/label 等，
  //    元素本身无 onclick/tabindex/role，但 CSS 标了 pointer 表示可点击）。
  const pointerCandidates: HTMLElement[] = []
  for (const el of root.querySelectorAll('div, span, li, label, p, em, i, b, strong')) {
    if (seen.has(el)) continue
    if (!(el instanceof HTMLElement)) continue
    if (getComputedStyle(el).cursor !== 'pointer') continue
    pointerCandidates.push(el)
  }
  // 父子去重：保留最外层 pointer 元素（点击父容器即覆盖整个可选项），
  // 跳过被 pointer 祖先包裹的子元素，避免同一选项出现多个编号。
  const pointerSet = new Set(pointerCandidates)
  for (const el of pointerCandidates) {
    let hasPointerAncestor = false
    let parent = el.parentElement
    while (parent !== null && parent !== root) {
      if (pointerSet.has(parent)) { hasPointerAncestor = true; break }
