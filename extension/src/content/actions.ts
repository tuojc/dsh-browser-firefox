...
export async function runAction(action: string, args: Record<string, unknown>, ctx: ActionContext): Promise<ActionResult> {
  switch (action) {
    ...
    case 'browser_forward':
      return historyAction(1)
    case 'browser_reload':
      // 先响应再 reload：reload 会卸载当前 content script 并销毁响应端口。
...
async function clickAction(args: Record<string, unknown>, ctx: ActionContext): Promise<ActionResult> {
  // 提供 selector 时用 CSS 选择器定位（绕过编号无法区分的场景）
  const selector = typeof args.selector === 'string' && args.selector !== '' ? args.selector : undefined
  if (selector !== undefined) {
    const selIndex = typeof args.index === 'number' && Number.isInteger(args.index) && args.index >= 0 ? args.index : 0
    const el = document.querySelectorAll(selector)[selIndex]
  ...
  if (el instanceof HTMLAnchorElement) {
    ...
    if (href.startsWith('http://') || href.startsWith('https://')) {
      // 跨文档导航：先响应再导航，避免导航卸载 content script 后销毁响应端口。
      setTimeout(() => { el.click() }, 0)
      return { text: `已点击链接 [${index}] "${name}"，页面正在跳转… 加载后请重新 browser_snapshot。` }
    }
    el.click()
    await settle()
...
function evaluateAction(args: Record<string, unknown>): ActionResult {
  ...
  switch (action) {
    case 'count':
      ...
      return { text: String(document.querySelectorAll(selector).length) }
    case 'getText': {
      if (selector === '') throw new ActionError('bad-args', 'selector 不能为空')
      const el = document.querySelector(selector)
      return { text: el?.textContent?.trim() ?? '未找到元素' }
    }
    case 'click': {
      if (selector === '') throw new ActionError('bad-args', 'selector 不能为空')
    ...
    case 'querySelectorAll': {
      if (selector === '') throw new ActionError('bad-args', 'selector 不能为空')
      const els = [...document.querySelectorAll(selector)].slice(0, 50)
      const items = els.map((el, i) => {
        const tag = el.tagName.toLowerCase()
        const text = (el.textContent ?? '').trim().slice(0, 80)
        const href = el instanceof HTMLAnchorElement ? el.href : undefined
        return href !== undefined ? `[${i}] <${tag}> "${text}" → ${href}` : `[${i}] <${tag}> "${text}"`
      })
      return { text: items.length > 0 ? items.join('\n') : '(无匹配元素)' }
    }
    default:
      throw new ActionError('bad-args', `未知 action: ${action}（支持 count/getText/click/setValue/querySelectorAll）`)
  }
}
